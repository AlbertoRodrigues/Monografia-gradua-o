...
porc_validacao=0.4
porc_teste=0.2
num_minimo_terminal=200
num_minimo_interno=300
prof_max=3
porc_min=0.4
tipo_poda=2
porc_modelos=0.4
num_minimo_modelos=3
num_min_obs_test_var=100
tipo_resposta="continuo"
#Separacao treino e teste
#tipos_separacao=amostragem aleatoria, validacao cruzada, um contra todas e treino-teste.
tipo_separacao="amostragem_aleatoria"
#Quantidade amostragens aleatórias realizadas
numero_amostragens=50
      ...
      for(k in 1:length(arvore))
      {
        if(length(arvore[[k]])>1)
        {
          ...
          for(p in indices_modelos)
          {
            ...
            { 
              modelo2=glm(y_treino~.,data=dados2,family=poisson(link="sqrt"))
              modelos[[2]]=modelo2
            }else if(p==3)
            {
            ...
            {
              modelo2=glm.nb(y_treino~.,data=dados2,link="sqrt")
              modelos[[5]]=modelo2
            }else if(p==6)
          {
