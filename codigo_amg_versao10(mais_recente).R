...
#Fazer otimização aleatória
num_de_validacoes=30
erro_busca_aleatoria=c()
desvio_padrao=c()
#Geração aleatória
hiperp_num_minimo_terminal=c(50,100,120,150,180,200,210,250)
hiperp_num_minimo_interno=c(180,200,210,250,300,500,1000,1500,2000)
hiperp_prof_max=c(2,3,4,5,6,7)
#hiperp_porc_min=runif(1)
hiperp_tipo_poda=2
#hiperp_porc_modelos=runif(1)
hiperp_num_minimo_modelos=c(2,3,4)
hiperp_num_min_obs_test_var=c(30,50,100,200,300,400,500,600)
...
for(v in 1:num_de_validacoes)
{
  ...
  #tipos_separacao=amostragem aleatoria.
  tipo_separacao="amostragem_aleatoria"
  #Quantidade amostragens aleatórias realizadas
  numero_amostragens=20
  #Vetor que terá todos os erros de teste
  erro_teste=c()
  if(tipo_separacao=="amostragem_aleatoria")
  {
    for(l in 1:numero_amostragens)
    {
      #Realizando a partição 
      #função dos indices da partição de treino  
      amostra=sample(1:nrow(x),ceiling(nrow(x)*porc_treino),replace = F)
      x_treino=x[amostra,]
      y_treino=y[amostra]
      x_teste=x[-amostra,]
      y_teste=y[-amostra]
...
tabela=rbind.data.frame(erro_busca_aleatoria[pos],desvio_padrao[pos],vetor_num_minimo_terminal[pos],
                 vetor_num_minimo_interno[pos],vetor_prof_max[pos]
                 ,vetor_porc_min[pos],vetor_tipo_poda[pos]
        ,vetor_porc_modelos[pos],vetor_num_minimo_modelos[pos],vetor_num_min_obs_test_var[pos])
#tabela=rbind.data.frame(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10)
tabela
require(xtable)
...
#Dados especificos de preços das casas
#quantidade de validações=6+6+3+5+5+1+6
